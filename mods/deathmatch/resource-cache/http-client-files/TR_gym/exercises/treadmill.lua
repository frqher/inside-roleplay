...
function Treadmill:create(...)
    local instance = {}
    setmetatable(instance, Treadmill)
...
function Treadmill:constructor(...)
...
function Treadmill:render()
    ...
    dxDrawText("Naciskaj przycisk sprintu aby przesuwać znacznik w prawo. Możesz regulować prędkość bieżni naciskając LPM i PPM.", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + guiInfo.h - 15/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "bottom", false, true)
...
-- States
function Treadmill:setPlayerFallOff()
    setPedAnimation(localPlayer, "gymnasium", self.workState == "walking" and "gym_walk_falloff" or "gym_tread_falloff", -1, false, false, false, false, 500)
    setElementData(localPlayer, "animation", {"gymnasium", self.workState == "walking" and "gym_walk_falloff" or "gym_tread_falloff", false, true})
...
function Treadmill:setPlayerSprinting()
    setPedAnimation(localPlayer, "gymnasium", "gym_tread_sprint", -1, true, false, false, false, 500)
    setElementData(localPlayer, "animation", {"gymnasium", "gym_tread_sprint", false, true})
    self.workState = "sprinting"
end

function Treadmill:setPlayerJogging()
    setPedAnimation(localPlayer, "gymnasium", "gym_tread_jog", -1, true, false, false, false, 500)
    setElementData(localPlayer, "animation", {"gymnasium", "gym_tread_jog", false, true})
    self.workState = "jogging"
end

function Treadmill:setPlayerWalking()
    setPedAnimation(localPlayer, "gymnasium", "gym_tread_walk", -1, true, false, false, false, 500)
    setElementData(localPlayer, "animation", {"gymnasium", "gym_tread_walk", false, true})
    self.workState = "walking"
end

function Treadmill:enterThreadmill()
    local x, y, z = getPosition(self.element, Vector3(0, -1.3, 1))
    local _, _, rot = getElementRotation(self.element)

    setElementPosition(localPlayer, x, y, z)
    setElementRotation(localPlayer, 0, 0, rot)

    setPedAnimation(localPlayer, "gymnasium", "gym_tread_geton", 2000, false, false, false, false)
    setElementData(localPlayer, "animation", {"gymnasium", "gym_tread_geton", false, true})
    self.timer = setTimer(self.func.setPlayerWalking, 2000, 1)
...
function startTreadmillWorkout(element)
    if guiInfo.workout then return end
    guiInfo.workout = Treadmill:create(element)
end