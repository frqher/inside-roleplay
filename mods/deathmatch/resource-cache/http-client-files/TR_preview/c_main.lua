...
addEventHandler("onClientResourceStart", getResourceRootElement( getThisResource()), function()
	if not isMTAUpToDate("07331") then
		outputChatBox('Nesne önizlemesi: MTA 1.5 istemcinizi güncelleyin. nightly.mtasa.com adresinden indirin',255,0,0)
		return
	end
	isMRTShaderSupported = vCardNumRenderTargets() > 1
	if not isMRTShaderSupported then
		outputChatBox('Nesne önizlemesi: Gölgelendiricide çoklu RT desteklenmiyor',255,0,0)
		return
	end
end)
