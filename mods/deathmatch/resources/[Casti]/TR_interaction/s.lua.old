...
function interactionTrigger(option, sound, index)
	...
	if option then
		...
		elseif option == 2 then
			if not exports.TR_vehicles:canUseVehicle(veh, client) then exports.TR_noti:create(client, "Nie posiadasz kluczyków do tego pojazdu.", "error") return end
			if not getVehicleEngineState(veh) then
				local vehData = getElementData(veh, "vehicleData")
				if not vehData then return end
				if vehData.fuel <= 0.5 then
					exports.TR_noti:create(client, "Nie możesz zapalić silnika, ponieważ skończyło ci się paliwo.", "error")
					return
				end
		...
		elseif option == 4 then
			if getElementData(veh, "blockAction") then exports.TR_noti:create(client, "Nie możesz tego zrobić, ponieważ trwa już jakaś interakcja.", "error") return end
			if isElement(getElementData(veh, "winchPlayer")) then exports.TR_noti:create(client, "Nie możesz tego zrobić gdy jesteś podpięty do wciągarki.", "error") return end
			if getElementData(veh, "wheelBlock") then exports.TR_noti:create(client, "Nie możesz tego zrobić, ponieważ masz założoną blokadę na koło.", "error") return end
			if isElementFrozen(veh) then
				...
				if getVehicleType(veh) == "Boat" then
					if getElementSpeedValue(veh, "km/h") > 1 then
						exports.TR_noti:create(client, "Nie możesz zarzucić kotwicy podczas płynięcia.", "error")
						return
					end
				else
					if getElementSpeedValue(veh, "km/h") > 0 then
						exports.TR_noti:create(client, "Nie możesz zaciągnąć ręcznego w trakcie jazdy.", "error")
						return
					end
				end
		...
		elseif option == 5 then
			if getElementSpeedValue(veh, "km/h") > 3 then
				exports.TR_noti:create(client, "Nie możesz wyrzucić nikogo podczas jazdy. Chcesz mieć kogoś na sumieniu?!", "error")
				return
			end

			local occupants = getVehicleOccupants(veh)
			occupants[0] = nil
			triggerClientEvent(occupants, "removePlayerFromVehicle", resourceRoot, index)
		...
		elseif option == 9 then
			...
			exports.TR_noti:create(client, "Osoba opłacająca przejazd została pomyślnie zmieniona.", "success")

		elseif option == 10 then
			local newState = not getElementData(veh, "neonEnabled")
			setElementData(veh, "neonEnabled", newState)
			exports.TR_noti:create(client, newState and "Neony zostały włączone." or "Neony zostały wyłączone.", "success")
		end
...
function interactWithPlayer(target, type)
	...
	if targetInteract then
		if targetInteract.target == client and targetInteract.type == type then
			if (getTickCount() - targetInteract.time)/10000 >= 1 then
				exports.TR_noti:create(client, string.format("Propozycja od gracza %s wygasła.", getPlayerName(target)), "info")
				...
				if math.abs(targetPos.z - clientPos.z) > 0.1 then
					exports.TR_noti:create(client, "Różnica wysokości jest zbyt duża by wykonać interakcję.", "error")
					triggerClientEvent(client, "updateInteraction", resourceRoot)
					return
				end
	...
	if plrInteract then
		if plrInteract.target == target and (getTickCount() - plrInteract.time)/10000 < 1 then
			exports.TR_noti:create(client, string.format("Gracz %s ma już oczekującą prośbę od ciebie.", getPlayerName(target)), "error")
			triggerClientEvent(client, "updateInteraction", resourceRoot)
			return
		end
	end
...
function vehicleOpen(vehicle)
	...
	triggerClientEvent(client, "updateInteraction", resourceRoot, false, false, "Nie posiadasz pilota do tego pojazdu.")
end
...
function openVehicleDoor(vehicle, type)
	...
	triggerClientEvent(client, "updateInteraction", resourceRoot, false, false, "Nie posiadasz kluczy do tego pojazdu.")
end
...
function flipVehicle(veh)
	...
	if exports.TR_admin:isPlayerOnDuty(plr) then
		...
		exports.TR_noti:create(plr, "Pojazd został pomyślnie obrócony.", "success")
		return
	end
	...
	exports.TR_noti:create(plr, "Zabierasz się do obrócenia pojazdu. Zajmie ci to 30 sekund.", "info")
...
function getElementSpeedValue(element, typ)
    assert(isElement(element), "Zła wartość " .. type(element) .. ")")
    ...
    assert((typ == nil or type(typ) == "string" or type(typ) == "number") and ((tonumber(typ) and (tonumber(typ) == 0 or typ == tonumber(typ) == 1 or tonumber(typ) == 2)) or typ == "m/s" or typ == "km/h" or typ == "mph"), "Nieprawidłowa prędkość")
...
function getInteractData(type)
	local data = {
		["welcome"] = {
			...
			targetText = "Gracz %s chce się z tobą przywitać. Użyj interakcji aby się zgodzić.",
			clientText = "Zaproponowałeś graczowi %s przywitanie się z tobą.",
		},
		["kiss"] = {
			dist = 1.08,
			animTime = 6500,
			...
			targetText = "Gracz %s chce się z tobą pocałować. Użyj interakcji aby się zgodzić.",
			clientText = "Zaproponowałeś graczowi %s pocałowanie się.",
		},
		["trade"] = {
			noti = "trade",

			targetText = "Gracz %s chce z tobą handlować. Użyj interakcji aby się zgodzić.",
			clientText = "Zaproponowałeś graczowi %s handel.",
		},
	}
	return data[type]
end
...
function removePlayerFromVehicle(veh, index)
	local plr = getVehicleOccupant(veh, index)
	if plr then
		triggerClientEvent(plr, "removePlayerFromVehicle", resourceRoot, index)
	end
	triggerClientEvent(client, "updateInteraction", resourceRoot)
end
addEvent("removePlayerFromVehicle", true)
addEventHandler("removePlayerFromVehicle", resourceRoot, removePlayerFromVehicle)
...
function giveVehicleGasoline(veh)
	...
	exports.TR_noti:create(client, "Pojazd został pomyślnie dotankowany.", "success")
	triggerClientEvent(client, "updateInteraction", resourceRoot)
end
...
function setVehicleUnfrozen(veh)
	...
	exports.TR_noti:create(client, "Hamulec ręczny został pomyślnie zdjęty.", "success")
	triggerClientEvent(client, "updateInteraction", resourceRoot)
end
...
function healMedicPlayer(plr)
	...
	exports.TR_noti:create(plr, string.format("Zostałeś uleczony przez %s.", getPlayerName(client)), "success")
	exports.TR_noti:create(client, string.format("Gracz %s został uleczony.", getPlayerName(plr)), "success")
end
