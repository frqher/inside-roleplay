...
function getUpdates(limit)
    ...
    if updates and updates[1] then
        ...
        triggerClientEvent(client, "updateLoginUpdates", resourceRoot, {{text = "Değişiklik listesini indiremediniz"}})
    end
end
...
function registerAccount(login, password, email, reference)
    ...
    if checkUsername and checkUsername[1] then
        triggerClientEvent(client, "loginResponseServer", resourceRoot, "Kullanıcı zaten böyle bir giriş yapmış.", "error")
        return
    end
    ...
    if checkSerial and #checkSerial == 2 then
        triggerClientEvent(client, "loginResponseServer", resourceRoot, "Hesap oluşturma sınırına ulaştınız. (2).", "error")
        return
    end
    ...
    if checkEmail and checkEmail[1] then
        triggerClientEvent(client, "loginResponseServer", resourceRoot, "Bu e-posta zaten bir hesaba atanmış.", "error")
        return
    end
    ...
    if reference then
        ...
        if tonumber(referenceUID) == nil or string.len(referenceUID) < 1 then
            triggerClientEvent(client, "loginResponseServer", resourceRoot, "Referans kodu yanlış.", "error", "reference")
            return
        end
    end
...
function loginAccount(login, password)
    ...
    if performLogin and performLogin[1] then
        if passwordVerify(password, performLogin[1]["password"]) then
            if isPlayerLogged(performLogin[1]["UID"]) then triggerClientEvent(client, "loginResponseServer", resourceRoot, "Bu hesap zaten oyunda.", "error") return end
            if checkBanAccount(performLogin[1]["UID"]) then triggerClientEvent(client, "loginResponseServer", resourceRoot, "Bu hesap yasaklanmıştır ve giriş yapamazsınız.", "error") return end
            ...
            exports.TR_mysql:querry("INSERT INTO `tr_logs` (player, text, serial, ip, type) VALUES (?, ?, ?, ?, ?)", performLogin[1].UID, "Hesaba başarıyla giriş yapıldı.", serial, ip, "login")
            exports.TR_mysql:querry("UPDATE `tr_accounts` SET `lastOnline` = NOW() WHERE UID = ? LIMIT 1;", performLogin[1].UID)
        else
            triggerClientEvent(client, "loginResponseServer", resourceRoot, "Girilen şifre yanlış", "error")
            exports.TR_mysql:querry("INSERT INTO `tr_logs` (player, text, serial, ip, type) VALUES (?, ?, ?, ?, ?)", performLogin[1].UID, "Yanlış şifre tespit edildi.", serial, ip, "login")
        end
    else
        triggerClientEvent(client, "loginResponseServer", resourceRoot, "Böyle bir hesap mevcut değil.", "error")
    end
end
...
function onLoadCharacterData(data, plrData)
    ...
    local pos = split(plrData[1].position or data.respawnPos, ",")
    ...
    setTimer(function(oyuncu)
        setElementPosition(oyuncu, pos[1], pos[2], pos[3])
    end, 1000, 1, client)
