...
function getPlayerRealName(plr, color, hasMask, isInZone)
	...
	if hasMask and isInZone then
		if admin then return string.format("Bilinmeyen #%03d (%s)", id, name), "#929ea8" end
	  	return string.format("Bilinmeyen #%03d", id), "#929ea8"
	end
...
function getPlayerIcons(plr)
	...
	if getPedArmor(plr) > 0 then table.insert(icons, "files/images/icons/armor.png") end
