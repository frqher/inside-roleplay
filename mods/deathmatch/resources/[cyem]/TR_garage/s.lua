...
function createDialogue()
	...
	exports.TR_npc:addDialogueText(garageData.dialogue, "Merhaba.", {pedResponse = "Merhaba. Nasıl yardımcı olabilirim?"})

	exports.TR_npc:addDialogueText(garageData.dialogue, "Park edilmiş aracımı almak istiyorum.", {pedResponse = "", img = "garage", trigger = "getVehiclesGarage", responseTo = "Merhaba."})
	exports.TR_npc:addDialogueText(garageData.dialogue, "Ne iş yaptığınızı öğrenmek istiyorum.", {pedResponse = "Sorun değil. Herhangi bir şey hakkında sorabilirsiniz. Elimden geldiğince tüm soruları cevaplayacağım.", responseTo = "Merhaba."})
	exports.TR_npc:addDialogueText(garageData.dialogue, "Rahatsızlık verdiğim için özür dilerim. Hoşça kalın.", {pedResponse = "Önemli değil. Hoşça kalın.", responseTo = "Merhaba."})

	exports.TR_npc:addDialogueText(garageData.dialogue, "Neleri depolayabilirim?", {pedResponse = "Anahtarınız olan her aracı depolayabiliriz. Teknik durumundan bağımsız olarak.", responseTo = "Ne iş yaptığınızı öğrenmek istiyorum."})
	exports.TR_npc:addDialogueText(garageData.dialogue, "Hizmetleriniz için ne kadar ödemem gerekecek?", {pedResponse = "Hizmetlerimiz tamamen ücretsizdir, çünkü devlet tarafından finanse ediliyoruz.", responseTo = "Ne iş yaptığınızı öğrenmek istiyorum."})
	exports.TR_npc:addDialogueText(garageData.dialogue, "Aracımda kesinlikle bir sorun olmayacak mı?", {pedResponse = "Bize emanet edilen araçlar, sadece çalışanların erişebildiği kapalı bir alanda saklanır. Güvenlik garanti altındadır.", responseTo = "Ne iş yaptığınızı öğrenmek istiyorum."})

	exports.TR_npc:addDialogueText(garageData.dialogue, "Hoşça kalın.", {pedResponse = "Hoşça kalın. İyi günler dilerim."})
...
function getVehiclesGarage(ped)
	...
	if not exports.TR_vehicles:canVehicleEnter(client) then
		triggerClientEvent(client, "showCustomMessage", resourceRoot, pedName, "Hiçbir aracı veremem çünkü araç kullanma yasağı aldınız.", "files/images/npc.png")
		return
	end
	triggerClientEvent(client, "showCustomMessage", resourceRoot, pedName, "Anlaşıldı. Sizi ne ilgilendiriyor?", "files/images/npc.png")
	...
	if querry and #querry > 0 then
		...
		exports.TR_noti:create(source, "Depolama alanında hiç aracınız bulunmamaktadır.", "error")
	end
end
...
function spawnGarageVehicle(data)
	...
	setTimer(function()
		...
		if getDistanceBetweenPoints3D(x, y, z, tonumber(garagePos[1]), tonumber(garagePos[2]), tonumber(garagePos[3])) < 5 then
			...
			exports.TR_noti:create(plr, "Araç, teslim alınan yerden ayrılmadığı için garaja bırakıldı.", "info", 5)
		else
