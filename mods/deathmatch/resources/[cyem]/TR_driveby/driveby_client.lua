...
function Driveby:render()
    ...
    if self:getElementSpeed(veh, 1) >= 60 then
        self:stopDriveby()
        return
    end
end
...
function Driveby:changeShooting(...)
    setPedControlState("vehicle_fire", arg[2] == "down" and true or false)
end
...
function Driveby:startDriveby()
    ...
    if self:getElementSpeed(vehicle, 1) >= 60 then return end
    ...
    if getVehicleType(vehicle) ~= "Bike" and getVehicleType(vehicle) ~= "BMX" and not isVehicleWindowOpen(vehicle, settings.seatWindows[seat]) and not settings.withoutRoof[vehicleID] then
        exports.TR_noti:create("Araçtan ateş etmek için önce pencereyi açmanız gerekmektedir.", "info")
        return
    end
