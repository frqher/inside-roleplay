...
function ParkingTakeout:constructor(...)
    ...
    self.buttons.submit = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h - 50/zoom, 235/zoom, 40/zoom, "Onayla")
...
function ParkingTakeout:render()
    ...
    dxDrawText("Cezanı öde", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(240, 196, 55, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")

    if self.vehicles[self.selected] then
        dxDrawText(string.format("Polis : %s\nKurtarma maliyeti : $%.2f\nSebep: %s", self.vehicles[self.selected].police, self.vehicles[self.selected].price, self.vehicles[self.selected].reason), guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + 50/zoom, guiInfo.x + guiInfo.w - 15/zoom, guiInfo.y + guiInfo.h - 15/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "top", true, true, false)
    else
        dxDrawText("Listeden bir araç seçin.", guiInfo.x + guiInfo.w - 245/zoom, guiInfo.y + guiInfo.h/2, guiInfo.x + guiInfo.w - 15/zoom, guiInfo.y + guiInfo.h/2, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.parts, "center", "center", false, false, false, true)
    end
...
function ParkingTakeout:buttonClick(...)
    ...
    elseif arg[1] == self.buttons.submit then
        ...
        if not data then exports.TR_noti:create("Öncelikle bir araç seçmelisiniz.", "error") return end
...
function ParkingTakeout:response(success)
    if success then
        exports.TR_noti:create("Araç polis otoparkından başarıyla çıkarıldı.", "success")
        self:close()
