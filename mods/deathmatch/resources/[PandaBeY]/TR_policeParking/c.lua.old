...
function PoliceParking:constructor(...)
    ...
    self.edits = {}
    self.edits.reason = exports.TR_dx:createEdit(guiInfo.x + (guiInfo.w - 400/zoom)/2, guiInfo.y + 120/zoom, 400/zoom, 40/zoom, "Wpisz powód odholowania")
    self.edits.price = exports.TR_dx:createEdit(guiInfo.x + (guiInfo.w - 250/zoom)/2, guiInfo.y + 170/zoom, 250/zoom, 40/zoom, "Wpisz cenę wykupienia")
    ...
    self.buttons.accept = exports.TR_dx:createButton(guiInfo.x + guiInfo.w - 260/zoom, guiInfo.y + guiInfo.h - 50/zoom, 250/zoom, 40/zoom, "Odłóż na parking")
...
function PoliceParking:render()
    ...
    dxDrawText("Parking policyjny", guiInfo.x, guiInfo.y, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(212, 175, 55, 255 * self.alpha), 1/zoom, self.fonts.main, "center", "center")
    dxDrawText(string.format("Holowany pojazd:\n%s (%d)", self.vehicleData.name, self.vehicleData.ID), guiInfo.x, guiInfo.y + 60/zoom, guiInfo.x + guiInfo.w, guiInfo.y + 50/zoom, tocolor(220, 220, 220, 255 * self.alpha), 1/zoom, self.fonts.veh, "center", "top")
    dxDrawText("Aby wyjąć pojazd gracz będzie musiał opłacić wpisaną powyżej kwotę. Pamiętaj, aby kary były zgodne z taryfikatorem!", guiInfo.x + 30/zoom, guiInfo.y + 210/zoom, guiInfo.x + guiInfo.w - 30/zoom, guiInfo.y + guiInfo.h - 50/zoom, tocolor(170, 170, 170, 255 * self.alpha), 1/zoom, self.fonts.info, "center", "center", true, true)
end
...
function PoliceParking:buttonClick(btn)
    ...
    elseif btn == self.buttons.accept then
        ...
        if string.len(reason) < 2 or string.len(reason) > 100 then exports.TR_noti:create("Powód odholowania powinien posiadać od 2 do 40 znaków.", "error") return end
        if not price then exports.TR_noti:create("Cena jest niepoprawna.", "error") return end
        if price < 0 then exports.TR_noti:create("Minimalna kwota wynosi $0.", "error") return end
        if price > 7000 then exports.TR_noti:create("Kwota nie może przekraczać $7000.", "error") return end
...
function PoliceParking:response(success)
    if success then
        exports.TR_noti:create("Pojazd został odłożony na parking policyjny.", "success")
        self:close()
    end
...
function canPlaceInParking(plr)
    ...
    if not id then exports.TR_noti:create("Na parking możesz wstawić tylko pojazd, który jest prywatny.", "error") return false end
...
function createPoliceParkingMarkers()
    for i, v in pairs(policeParkings) do
        ...
        setElementData(marker, "markerData", {
            ...
            desc = "Wjedź w marker holując pojazd aby go wstawić na parking policyjny."
        }, false)
