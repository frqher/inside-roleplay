...
function removePlayerFromTaxi(vehicle)
    triggerClientEvent(getVehicleOccupants(vehicle), "removePedFromVehicle", resourceRoot)
end
...
function addTaxiRequest()
    if taxiRequests[client] then exports.TR_noti:create(client, "Zaten bir taksi bekliyorsunuz.", "error") return end
    taxiRequests[client] = {getElementPosition(client)}

    exports.TR_noti:create(client, "Bir taksi çağrıldı. Taksi şoförünün işini kolaylaştırmak için fazla uzağa gitmeyin.", "success")
...
function selectTaxiRequest(player)
    if taxiRequests[player] then
        ...
        if isElement(player) then
            triggerClientEvent(player, "showCustomMessage", resourceRoot, string.format("#d89932Taksi Sürücüsü [%d] %s", getElementData(client, "ID"), getPlayerName(client)), "#ac7a28Başvuru kabul edildi. Taksi yakında belirtilen yerde olacaktır.", "files/images/msg_received.png")
        end
    else
...
function declineTaxiRequest(player)
    ...
    if isElement(player) then
        triggerClientEvent(player, "showCustomMessage", resourceRoot, string.format("#d89932Taksi Sürücüsü [%d] %s", getElementData(client, "ID"), getPlayerName(client)), "#ac7a28Çok üzgünüm, ancak başvurunuzu iptal etmek zorunda kaldım ve verdiğiniz adreste görünmeyeceğim. Rahatsızlıktan dolayı özür dileriz..", "files/images/msg_received.png")
    end
end
